...
def make_sitemap_for_model(model:Type[models.Model], changefreq:str="never", priority:float=0.5)->Type[Sitemap]:
...
def sitemap_for_model(model:models.Model, **kwargs)->Type[Sitemap]:
    return GLOBAL_SITEMAPS.setdefault(model.__name__, make_sitemap_for_model(model, **kwargs))
